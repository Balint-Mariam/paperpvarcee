...
ROOT <- getwd()
FINAL <- file.path(ROOT, "FINAL_Q1_PAPER_OUTPUTS")
OUT_DIR <- file.path(FINAL, "07_methodological_code_audit")
...
DATA_FILE <- file.path(FINAL, "01_data", "model_ready_dataset.xlsx")
TRANSFORM_FILE <- file.path(FINAL, "01_data", "transformation_summary.xlsx")
MASTER_FILE <- file.path(FINAL, "02_master_excel", "MASTER_all_tables_for_paper.xlsx")
APPENDIX_FILE <- file.path(FINAL, "02_master_excel", "MASTER_appendix_tables.xlsx")
PREDIAG_FILE <- file.path(FINAL, "02_master_excel", "pre_model_diagnostics_cleaned.xlsx")
DK_FILE <- file.path(FINAL, "06_alternative_inference_DK", "FE_LSDV_PVAR_DK_inference.xlsx")
FIG_MANIFEST <- file.path(FINAL, "03_figures", "figure_manifest_polished.xlsx")
TABLE_MANIFEST <- file.path(FINAL, "02_master_excel", "table_manifest_polished.xlsx")
...
for (sf in script_files[file.exists(script_files)]) {
...
if (nrow(dk_coef) > 0) {
  ...
  report_txt <- paste(file_text(file.path(FINAL, "06_alternative_inference_DK", "DK_inference_report.md")), collapse = "\n")
...
add_check("figures_tables_checks", "M04", "Reduced-form Table 3 should use DK stars when paper reports DK inference", "DK table available in 06_alternative_inference_DK", file.exists(DK_FILE), "", file.exists(DK_FILE), severity = "medium", notes = "Use DK inference workbook for manuscript Table 3 if coefficient-level DK inference is reported.")
add_check("figures_tables_checks", "M05", "Structural figures are not modified by DK inference", "DK outputs isolated in folder 06", dirname(DK_FILE), "", grepl("06_alternative_inference_DK", DK_FILE), severity = "medium")
