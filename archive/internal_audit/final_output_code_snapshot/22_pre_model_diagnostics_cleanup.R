...
DIAG_FILE <- file.path(FINAL_DIR, "02_master_excel", "pre_model_diagnostics_cleaned.xlsx")
UPDATED_MASTER_FILE <- file.path(FINAL_DIR, "02_master_excel", "MASTER_all_tables_for_paper_updated_diagnostics.xlsx")
...
sample_overview <- function(dat) {
  ...
  tibble::tibble(
    source_file = normalizePath(DATA_FILE, winslash = "/", mustWork = FALSE),
...
write_report <- function(path, sample_info, unit_root_summary, cips_results, cd_results, final_verdict, notes) {
  ...
  lines <- c(
    ...
    "## Output Files",
    "",
    paste0("- ", normalizePath(DIAG_FILE, winslash = "/", mustWork = FALSE)),
    paste0("- ", normalizePath(UPDATED_MASTER_FILE, winslash = "/", mustWork = FALSE)),
    paste0("- ", normalizePath(REPORT_FILE, winslash = "/", mustWork = FALSE))
  )
